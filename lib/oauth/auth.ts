...
export const createState = (): string => {
  // Match the state format emitted by the current Codex CLI: 32 random bytes
  // encoded as unpadded base64url (43 characters). The OAuth gateway validates
  // this value as part of its browser-session verifier flow.
...
export const createAuthorizationFlow = async (): Promise<AuthorizationFlow> => {
  ...
  // URLSearchParams serializes spaces as '+'. Codex CLI emits RFC 3986
  // percent-encoding, so preserve that wire format exactly.
  return { pkce, state, url: url.toString().replaceAll("+", "%20") };
};
...
export const extractAccountId = (accessToken: string): string | null => {
  ...
  const organizationId = payload.organizations?.find(
    (organization) => typeof organization?.id === "string" && organization.id.length > 0,
  )?.id;
  if (organizationId) {
 ...
 */
export const extractAccountIdFromTokens = (
