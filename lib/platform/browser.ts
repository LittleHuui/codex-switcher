...
export const getBrowserLauncher = (
  ...
  if (platform === "win32") {
    return {
      // Do not use `cmd /c start` here. OAuth URLs contain `&`, which cmd
      // treats as command separators unless its quoting survives every layer
      // of process creation. The Windows URL protocol handler receives the
      // complete URL as one direct argument and invokes the default browser.
