...
- First community-maintained release, based on upstream `1.8.6`.
- Add a `NOTICE` file and publish it with the npm package to preserve upstream
  attribution and clearly identify this project as an unofficial fork.
- Add `THIRD_PARTY_NOTICES.md` to document direct runtime dependency licenses
  and publish it with the npm package.
- Refuse npm publication while `name`, `author`, or `repository` still point
  to the upstream project.

### Fixes

- Windows OAuth login: open authorization URLs through the Windows URL protocol
  handler so `&` query separators are not interpreted by `cmd` and the browser
  receives the complete URL.
- Update the Codex OAuth authorization request with the current connector
  scopes, 32-byte base64url state, and Codex Desktop originator context.
- Write Codex `auth.json` using ChatGPT OAuth mode and prefer the ChatGPT
  account ID carried by the ID token.
- Replace the PKCE-only `@openauthjs/openauth` dependency with a local RFC 7636
  S256 implementation using Node's built-in cryptography.

### Documentation

- Rewrite installation, OAuth, troubleshooting, and release documentation in
  Chinese; clearly label this repository as an unofficial community fork.
